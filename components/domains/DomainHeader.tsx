...
const DomainHeader = (
   ...
   const isBacklinks = router.pathname === '/domain/backlinks/[slug]';
   ...
   return (
      ...
      <div className='flex w-full justify-between mt-4 lg:mt-0'>
         <ul className=' max-w-[270px] overflow-auto flex items-end text-sm relative top-[2px] lg:max-w-none'>
            ...
            <li className={`${tabStyle} ${router.pathname === '/domain/backlinks/[slug]' ? 'bg-white border border-b-0 font-semibold' : ''}`}>
               <Link href={`/domain/backlinks/${domain.slug}`} className='px-4 py-2 inline-block'>
                  <Icon type="link" size={13} classes='hidden lg:inline-block' />
                     ...
                     color="#aaa"
                     classes="ml-2 hidden lg:inline-block"
                     title='Discover pages that link to your domain'
                     />
               </Link>
            </li>
         </ul>
         <div className={'flex mb-0 lg:mb-1 lg:mt-3'}>
            {!isInsight && !isBacklinks && <button className={`${buttonStyle} lg:hidden`} onClick={() => setShowOptions(!showOptions)}>
               <Icon type='dots' size={20} />
            </button>
            }
            {(isInsight || isBacklinks) && <button className={`${buttonStyle} lg:hidden invisible`}>x</button>}
            <div
            ...
            style={{ display: showOptions ? 'block' : undefined }}>
               ...
               {!isConsole && !isInsight && !isIdeas && !isBacklinks && (
                  <button
